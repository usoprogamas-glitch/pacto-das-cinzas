...
| 2 | **Fluxo de campanha (atos)** | — | 🟡 quase done | Encadeamento funcional: intro→map_select (gating por ato)→estágios→chefe completa ato→save. Epílogo/credits implementado (`epilogue.tscn`, `game_completed` no CampaignSystem, roteamento pós-vitória). Falta: cutscenes narrativas entre atos |
