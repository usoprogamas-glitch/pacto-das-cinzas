...
### 13. `battle_scene.gd` god file ✅ RESOLVIDO (2026-09-07: spawner + hud + flow + input + systems extraídos)
- 2041 linhas concentrando spawn/UI/combate/campanha. **Extraídos com wrappers de delegação** (lazy RefCounted acessando a cena via `battle.`; API dos testes preservada): `battle_spawner.gd` (#13a, spawn/sprites/HP bars), `battle_hud.gd` (#13b, setup_ui/HUDs/forge/cook), `battle_flow.gd` (#13c, turnos/resultado/progressão/naming), `battle_input.gd` (#13d, cliques/seleção/timed hit/block/menus), `battle_systems.gd` (#13e, Kaelen HUD/balance/boss bar/progressão/formas §8/forja/cozinha/taverna/travessia/combo). → battle_scene **426 linhas**. **Regra cumprida**: 1 módulo por commit com a suíte verde. Lições: wrappers de funções com retorno precisam `return`; script Python de extração exige lista explícita prefixo/interno e revisão de inferência `:=` sobre chamadas dinâmicas.
