...
### 13. `battle_scene.gd` god file 🟡 QUASE LÁ (2026-09-07: spawner + hud + flow + input extraídos)
- 2041 linhas concentrando spawn/UI/combate/campanha. **Extraído (2026-09-06)**: `scripts/battle/battle_spawner.gd` (RefCounted, ~340 linhas — setup_battle, pool data-driven, party/souls, create_unit/sprites/HP bar, binding de bosses) com **wrappers de delegação** na cena preservando a API dos testes (lazy: testes instanciam a cena sem _ready e stubam grid/unit_container).battle_scene → 1701 linhas. **Extraído (2026-09-06 #13b)**: `scripts/battle/battle_hud.gd` (~500 linhas — setup_ui, HUDs de Kaelen/combo/balance/boss/progressão, painel de ações, forge/cook panels; handlers permanecem na cena). **Extraído (2026-09-06 #13c)**: `scripts/battle/battle_flow.gd` (~230 linhas — check_all_units_acted/fases/turnos, eventos de unidade, check_battle_end, _on_battle_won/lost, _commit_progression, naming seam, victory/defeat screens, destinos pós-vitória). **Extraído (2026-09-07 #13d)**: `scripts/battle/battle_input.gd` (~200 linhas — handle_event (roteamento de `_input`), cliques, select/deselect, move/attack com selecionada, timed hit/block, menus contextuais, _on_move/attack/wait) → battle_scene **1013 linhas**. **Regra**: 1 módulo por commit com a suíte verde. Restante: handlers de sistemas (Kaelen/taverna/cozinha/travessia) → candidato `battle_systems.gd`; excitação §6-7.
