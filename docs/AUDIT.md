...
### 13. `battle_scene.gd` god file 🟡 EM PROGRESSO (2026-09-06: spawner + hud + flow extraídos)
- 2041 linhas concentrando spawn/UI/combate/campanha. **Extraído (2026-09-06)**: `scripts/battle/battle_spawner.gd` (RefCounted, ~340 linhas — setup_battle, pool data-driven, party/souls, create_unit/sprites/HP bar, binding de bosses) com **wrappers de delegação** na cena preservando a API dos testes (lazy: testes instanciam a cena sem _ready e stubam grid/unit_container).battle_scene → 1701 linhas. **Extraído (2026-09-06 #13b)**: `scripts/battle/battle_hud.gd` (~500 linhas — setup_ui, HUDs de Kaelen/combo/balance/boss/progressão, painel de ações, forge/cook panels; handlers permanecem na cena). **Extraído (2026-09-06 #13c)**: `scripts/battle/battle_flow.gd` (~230 linhas — check_all_units_acted/fases/turnos, eventos de unidade, check_battle_end, _on_battle_won/lost, _commit_progression, naming seam, victory/defeat screens, destinos pós-vitória) → battle_scene **913 linhas**. **Regra**: 1 módulo por commit com a suíte verde. Restante: handlers de interação/seleção e sistema de excitação (§6-7).
